...
export default function ModalTermsAndCondition({onOpen, onClose}) {
    if (!onOpen) return null
  return (
    <div className='bg-textFadeBlack fixed inset-0 z-50'>
        <div className='flex flex-col h-screen justify-center items-center py-4'>
            <div className='sm:max-w-[450px] max-w-[340px] h-[450px] tablet:h-[600px] border-solid border-[1px] rounded-[5px] px-5 py-4 bg-[#FFFFFF] overflow-auto p-4 text-justify'>
            ...
            <button className='w-full sm:max-w-[450px] max-w-[340px] rounded-[5px] px-5 py-2 bg-lime-600 p-2 mt-2 text-base text-primary hover:bg-lime-700' onClick={onClose}>Close</button>
        </div>
    </div>
  )
}
