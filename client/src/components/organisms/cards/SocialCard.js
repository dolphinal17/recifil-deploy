...
export default function SocialCard({ image, name, author, proimg }) {
  return (
    <div className="w-[14.5rem] h-[18.5rem] rounded-md shadow-[0_3px_10px_rgb(0,0,0,0.2)]">
      ...
      <div
        className={`w-full h-[4rem] rounded-b-md p-[0.75rem] drop-shadow-md flex justify-between items-center`}
      >
        <div className="flex flex-col ">
          ...
          <div className="flex flex-row items-center gap-2 mb-2">
            <img src={proimg} className="w-[2.1rem] h-[2.1rem] border-black rounded-full" style={{
              border: '1px solid green',
            }}></img>
            <label className="text-sm font-light tablet:font-normal text-fadeBlack">
              {author}
            </label>
          </div>
        </div>
